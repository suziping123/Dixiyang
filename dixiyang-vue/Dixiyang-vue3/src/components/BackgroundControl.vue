  ...
  <div class="background-control" :class="[`mode-${mode}`]">
    <div v-if="mode === 'compact'" class="compact-mode">
      <button v-for="t in THEMES" :key="t.id" class="theme-btn"
        :class="{ active: bgConfig.themeId.value === t.id }"
        :title="t.label" @click="bgConfig.setTheme(t.id)">{{ t.icon }}</button>
    </div>

    <div v-else class="full-mode">
      <div class="settings-section">
        <h3 class="section-title">全局主题</h3>
        <p class="section-desc">影响所有页面的完整配色</p>
        <div class="theme-grid">
          <button v-for="t in THEMES" :key="t.id" class="theme-card"
            :class="{ active: bgConfig.themeId.value === t.id }"
            @click="bgConfig.setTheme(t.id)">
      ...
      <div v-if="bgImageList.length" class="settings-section">
        <h3 class="section-title">背景图</h3>
        <p class="section-desc">叠加在背景之上的预设图（QQ 风格·半透明模糊）</p>
        <div class="bg-image-grid">
          <button v-for="img in bgImageList" :key="img.id" class="bg-image-card"
          ...
          <button class="bg-image-card no-bg" :class="{ active: !bgConfig.bgImageId.value }"
...
onMounted(async () => {
  for (const img of BG_IMAGES) {
    try { loaded.value[img.id] = await img.importFn() } catch { /* skip */ }
  }
})

const handleReset = () => {
  if (confirm('恢复默认主题和背景？')) {
...
.theme-card.active { border-color: var(--accent-primary); background: rgba(59,130,246,0.10); box-shadow: 0 0 16px rgba(59,130,246,0.15); }
.theme-icon { font-size: 2rem; line-height: 1; }
.theme-info { display: flex; flex-direction: column; gap: 2px; }
.theme-name { font-weight: 600; font-size: 1rem; color: var(--text-on-card); }
...
.btn-reset { width: 100%; padding: 12px; border: 1px solid var(--surface-glass-border); border-radius: 10px; background: var(--surface-input); color: var(--text-secondary); cursor: pointer; font-weight: 600; transition: all 0.3s; }
.btn-reset:hover { background: rgba(168,85,247,0.12); border-color: rgba(168,85,247,0.4); color: var(--accent-purple); }
